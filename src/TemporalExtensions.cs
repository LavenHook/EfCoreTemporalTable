...
namespace EfCoreTemporalTable
{
  ...
  public static class TemporalExtensions
  {
    ...
    private static IQueryable<T> AsTemporal<T>(this DbSet<T> dbSet, string temporalCriteria, params object[] arguments) where T : class
    {
      ...
      var properties = string.Join(",", dbSet
          .GetService<ICurrentDbContext>()
          .GetPropertyNames<T>());

      var selectSql = $"SELECT {properties} FROM {table}";
    ...
    private static IEnumerable<string> GetPropertyNames<T>(this ICurrentDbContext dbContext)
    {
      var entityType = dbContext.Context.Model.FindEntityType(typeof(T));
      var properties = entityType.GetProperties().Select(p => p.GetColumnName()?.GetSqlSafeName());
      return properties;
    }
